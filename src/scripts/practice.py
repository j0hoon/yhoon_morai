...
class ManeuverPlanner():
    def __init__(self):
        ...
        self.sub_Object_2 = rospy.Subscriber('/lidar_track2', Lidar_track2, self.callback_object)
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            object_class_2, object_status_x_2, object_status_y_2, object_status_d_2,object_status_rel_x_2, object_status_rel_y_2  = self.sub_Objectstatus(self.Object_status)

            # Calculate min distance
            # dist_1 = np.sqrt((ego_status_x - object_status_x_1)**2 + (ego_status_y - object_status_y_1)**2)
            # dist_2 = np.sqrt((ego_status_x - object_status_x_2)**2 + (ego_status_y - object_status_y_2)**2)
            # if dist_1 < dist_2:
            ...
            if mission_number == 0 :
                ...
                if object_status_rel_x_1<0 and object_status_rel_y_1>0 and object_status_d_1< 1.5:
                    self.maneuver_mode = 6 # acc
                    self.mode_pub.publish(self.maneuver_mode)  
                    print("maneuver_mode + acceration : ", self.maneuver_mode) 
                    rate.sleep()     
                
                elif object_status_rel_x_1>0 and object_status_rel_y_1>0 and object_status_d_1<1.5:
                    self.maneuver_mode = 2
                    self.mode_pub.publish(self.maneuver_mode)
                    print("maneuver_mode + deceleration : ", self.maneuver_mode)
                    rate.sleep()
                else:
                    self.maneuver_mode = 1 # acc
                    self.mode_pub.publish(self.maneuver_mode)  
                    print("maneuver_mode : ", self.maneuver_mode) 
                    rate.sleep()                     
                ...
                object_state                                    = self.RecognitionObject(lane_offset, lane_curvature, object_status_x_1, ego_status_x, object_status_y_1, ego_status_y)
                ...
                print("IVT", IVT, "MSD", MSD, "RISK_F", RISK_F)
                self.mode_pub.publish(self.maneuver_mode)
                rate.sleep()
