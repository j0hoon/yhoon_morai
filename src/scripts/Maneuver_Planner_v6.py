            ...
            if mission_number == 0 :
                ...
                if rotary_mode == 1 or rotary_mode == 4:
                    if object_status_d_front > 1.5:
                        self.maneuver_mode = 7 # init = 6
                        self.mode_pub.publish(self.maneuver_mode)  
                        print("maneuver_mode + acceration : ", self.maneuver_mode) 
                        rate.sleep()
                    elif object_status_d_front <= 1.5 and object_status_d_front > 0.8:
                ...
                # elif rotary_mode == 2 :
                #     if object_status_d_back < 1:
                #         self.maneuver_mode = 6 # acc
                #         self.mode_pub.publish(self.maneuver_mode)  
                #         print("maneuver_mode + acceration : ", self.maneuver_mode) 
                #         rate.sleep()
                #     elif object_status_d_back <= 1.5 and object_status_d_back > 1:
                ...
                risk,IVT, MSD, RISK_F                           = self.threatAssessment(D_nearest, ego_status_velocity)
                object_state                                    = self.RecognitionObject(lane_offset, lane_curvature_for_curve, object_status_x, ego_status_x, object_status_y, ego_status_y)
                self.maneuver_mode                                   = self.ManeuverMode(risk, object_state, trafficlight_signal)
                ...
                risk,IVT, MSD, RISK_F                           = self.threatAssessment(D_nearest, ego_status_velocity)
                object_state                                    = self.RecognitionObject(lane_offset_for_mission_5, lane_curvature, object_status_x_1, ego_status_x, object_status_y_1, ego_status_y)
                self.maneuver_mode                                   = self.ManeuverMode(risk, object_state, trafficlight_signal)
                ...
                # object_status_rel_y = object_status_rel_y_1
                '''
                if object_status_d == 0 or object_status_d >= 1.5:
                    self.maneuver_mode = 1 
                    self.mode_pub.publish(self.maneuver_mode)
                    print("maneuver_mode : ", self.maneuver_mode)
                    rate.sleep()
                elif  object_status_d < 1.5 and object_status_d != 0:
                    self.maneuver_mode = 4
                    self.mode_pub.publish(self.maneuver_mode)  
                    print("maneuver_mode : ", self.maneuver_mode) 
                    rate.sleep()
                ...
                if (k_object_status_d - k_1_object_status_d < 0) and k_object_status_d<1.5:
                    self.maneuver_mode = 4
                    self.mode_pub.publish(self.maneuver_mode)
                    print("maneuver_mode : ", self.maneuver_mode)
                    rate.sleep()
                #elif (k_object_status_d - k_1_object_status_d < 0) and k_object_status_d>1.5:
